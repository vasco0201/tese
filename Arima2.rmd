...
```{r}
cmp_table = setNames(data.frame(matrix(ncol = 5, nrow = 0)), c("Filename","Model Params","MAPE", "MAE","RMSE"))
data = read.csv("freeway_data/freeway_data2.csv")
filenames = list.files(path = "/home/vasco/tese/freeway_data", full.names = TRUE)
filenames_short = list.files(path = "/home/vasco/tese/freeway_data")
...
cmp_table[nrow(cmp_table) + 1,] = list(filenames_short[i],model_coef,mapeError,maeError,rmsError)
save(cmp_table,file="/home/vasco/cmp_table.RData")
}
...
```{r}
pred_table = setNames(data.frame(matrix(ncol =4, nrow = 0)), c("Pred Nr", "Mape","Pred","Obs"))
data = read.csv("freeway_data/freeway_data2.csv")
colnames(data) = c("Date","Count")
data$Time <-format(as.POSIXct(data$Date) ,format = "%H:%M:%S")
data$Date <- as.Date(data$Date)
